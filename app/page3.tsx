...
export default function Home() {
  ...
  const sahpraCount = devices.filter(d =>
  ...
  background:
    radial-gradient(900px 500px at 80% -10%, rgba(31,111,235,.10), transparent 55%),
  ...
  height:40px;
  border-radius:12px;
  background:linear-gradient(135deg,rgba(0,80,144,.10),rgba(127,183,214,.18));
  border:1px solid rgba(0,80,144,.18);
  display:grid;
  place-items:center;
...
.heroCopy p{
  ...
  border-radius:11px;
  background:linear-gradient(135deg,rgba(0,80,144,.10),rgba(127,183,214,.18));
  border:1px solid rgba(0,80,144,.12);
  display:grid;
  place-items:center;
  flex-shrink:0;
        }
        ...
        .badge.ok{
          ...
          border-color:rgba(19,122,59,.15);
        }
        .badge.warn{
          background:var(--warnBg);
          color:var(--warnText);
          border-color:rgba(161,92,0,.15);
        }
        .badge.danger{
          background:var(--dangerBg);
          color:var(--dangerText);
          border-color:rgba(159,29,29,.15);
        }
        .badge.neutral{
          background:#f1f5f9;
          color:#334155;
  ...
  font-size:13px;
  color:#fff;
  background:linear-gradient(180deg,#0080A0,#005090);
  box-shadow:0 6px 14px rgba(0,80,144,.18);
  border:none;
  cursor:pointer;
  transition:filter .15s;
  white-space:nowrap;
            ...
            <a className="brand" href="/">
              ...
              <div>
                <div className="brandName">
                  ALETIA <span style={{ color: 'var(--blue)', fontWeight: 800 }}>INDEX</span>
                </div>
            ...
            <section className="card cardPad">
              <div className="heroBlock">
                <div className="heroCopy">
                  <div className="eyebrow">Structured assurance for digital health</div>
                  <h1>Assess digital health tools beyond marketing claims</h1>
                  <p>
                    Regulatory, technical, and lifecycle signals in one structured index for
                    clinicians, buyers, and digital health companies.
                  </p>
                </div>

                <div className="actionRow">
                  <a href="/claim-profile" className="actionBtn actionBtnPrimary">Claim profile</a>
                  <a href="/submit-tool" className="actionBtn">Submit company/tool</a>
                  <a href="/request-review" className="actionBtn">Request verification</a>
                </div>

                <div className="proofRow">
                  <span>FDA coverage live</span>
                  <span>MHRA coverage live</span>
                  <span>SAHPRA data expansion underway</span>
                </div>
              </div>
                          ...
                          <tr key={device.device_id} onClick={() => setSelected(device)}>
                              ...
                              <div className="rowTitle">
                                <div className="appIcon">
                                  <svg viewBox="0 0 24 24" fill="none" width="20" height="20">
                                    <path
                                      d="M12 3 4 21h4l1.3-3h5.4L16 21h4L12 3Zm1.6 10H10.4L12 8.6 13.6 13Z"
                                      fill="#1f6feb"
                                    />
                                  </svg>
                                </div>
                            ...
                            <td>
                              <div style={{ fontSize: '13px', fontWeight: 600 }}>
                                Reviewed {formatDate(device.last_clinical_review)}
                              </div>
                              <div className="small">
  ...
  <div className="card cardPad">
    ...
    <div className="donutWrap">
      ...
      <div className="legend">
        ...
        <div className="legendRow"><span className="swatch" style={{background:'#0080A0'}}></span>{amberCount} • Amber</div>
        ...
        <div className="legendRow"><span className="swatch" style={{background:'#1f6feb'}}></span>{sahpraCount} • SAHPRA</div>
      </div>
    </div>
  </div>
  ...
  <div className="card cardPad">
    ...
    <div className="pills" style={{ marginTop: '8px' }}>
      {['SaMD', 'FDA Cleared', 'CE Marked', 'SAHPRA', 'Mental Health'].map(f => (
        <span key={f} className="pill" onClick={() => setSearch(f)}>
          {f}
        </span>
      ))}
    </div>
    ...
    <p style={{ fontSize: '12.5px', color: 'var(--muted)', lineHeight: 1.55, marginTop: '6px' }}>
      The Aletia Index provides independent clinical assurance for AI/ML medical devices.
      Data is verified by our clinical team against the 10-Point Assurance Checklist.
    </p>
  </div>
</aside>
          </div>
          ...
          <div className="footerGrid">
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                ...
                <div>
                  <div style={{ fontWeight: 800, color: 'var(--text)', letterSpacing: '.3px', fontSize: '14px' }}>
                    Aletia Index
                  </div>
              ...
              <p style={{ maxWidth: '60ch', lineHeight: 1.55 }}>
                We independently evaluate health technologies against structured assurance criteria.
                We do not endorse, certify, or validate clinical outcomes.
              </p>
            ...
            <div className="modal-header">
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: '7px', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap' }}>
                  ...
                  <span className={riskBadge(selected.health_status).cls}>
                    {riskBadge(selected.health_status).label} Risk
                  </span>
                </div>
                <h2 style={{ fontSize: '19px', fontWeight: 900, lineHeight: 1.2 }}>
                  {selected.manufacturers?.name}
                </h2>
                <p style={{ color: 'var(--muted)', fontSize: '13px', marginTop: '4px' }}>
                  {selected.manufacturers?.hq_location}
                </p>
              </div>
            ...
            <div className="modal-body">
              ...
              <div className="modal-section">
                ...
                {selected.regional_registrations?.map(r => (
                  <div key={`${r.country}-${r.regulatory_body}-${r.clearance_type}`} className="modal-row" style={{ marginTop: '8px' }}>
                  ...
                  <div className="modal-section">
                    ...
                    <div className="modal-grid" style={{ marginTop: '8px' }}>
                      <div className="modal-row">
                        <span style={{ fontSize: '13px', color: 'var(--muted)' }}>API Type</span>
                        <span style={{ fontSize: '13px', fontWeight: 600 }}>{selected.tech_specs.api_type}</span>
                      </div>
                      <div className="modal-row">
                        <span style={{ fontSize: '13px', color: 'var(--muted)' }}>Hosting</span>
                        <span style={{ fontSize: '13px', fontWeight: 600 }}>{selected.tech_specs.data_hosting}</span>
                      </div>
                      <div className="modal-row">
                        <span style={{ fontSize: '13px', color: 'var(--muted)' }}>FHIR</span>
                        <span style={{ fontSize: '13px', fontWeight: 600 }}>
                          {selected.tech_specs.fhir_compatible ? '✓ Yes' : '✗ No'}
                        </span>
                      </div>
                      <div className="modal-row">
                        <span style={{ fontSize: '13px', color: 'var(--muted)' }}>POPIA</span>
                        <span style={{ fontSize: '13px', fontWeight: 600 }}>
                          {selected.tech_specs.popia_compliant ? '✓ Compliant' : '✗ No'}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="modal-grid">
                <div>
                  ...
                  <p style={{ fontSize: '14px', fontWeight: 600 }}>
                    {formatDate(selected.last_automated_sync)}
                  </p>
                </div>
                <div>
                  <div className="modal-label">Last Clinical Review</div>
                  <p style={{ fontSize: '14px', fontWeight: 600 }}>
                    {formatDate(selected.last_clinical_review)}
                  </p>
                </div>
              </div>
            </div>
