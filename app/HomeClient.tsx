...
// Real FDA/MHRA devices store the name directly on device_master as manufacturer_name.
// Manually-seeded devices use the manufacturers FK join. This resolves both.
function mfrName(d: Device): string {
  return d.manufacturers?.name ?? d.manufacturer_name ?? d.device_id
}
  ...
  useEffect(() => {
    ...
    if (search) {
      result = result.filter(d =>
        ...
        mfrName(d).toLowerCase().includes(search.toLowerCase()) ||
                      ...
                      return (
                        <tr key={device.device_id}>
                          ...
                          <td style={{textAlign:'right'}}>
                            <button className="reportBtn" onClick={() => setSelected(device)}>
                              Quick view
                            </button>
                          </td>
                        </tr>
              ...
              <div className="card cardPad">
                ...
                <div className="donutWrap">
                  <svg width="120" height="120" viewBox="0 0 120 120" style={{ flexShrink: 0 }}>
                    ...
                    <circle cx="60" cy="60" r="40" fill="none" stroke="#005090" strokeWidth="16"
                      strokeDasharray={`${greenDash} ${circ}`} strokeDashoffset="0" transform="rotate(-90 60 60)" />
                    <circle cx="60" cy="60" r="40" fill="none" stroke="#0A8FBC" strokeWidth="16"
                      strokeDasharray={`${amberDash} ${circ}`} strokeDashoffset={`-${greenDash}`} transform="rotate(-90 60 60)" />
                    <circle cx="60" cy="60" r="40" fill="none" stroke="#7FB7D6" strokeWidth="16"
                      strokeDasharray={`${redDash} ${circ}`} strokeDashoffset={`-${greenDash + amberDash}`} transform="rotate(-90 60 60)" />
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div style={{flex:1}}>
                ...
                <h2 style={{fontSize:'19px',fontWeight:900,lineHeight:1.2}}>{mfrName(selected)}</h2>
            ...
            <div className="modal-body">
              ...
              <div style={{marginTop:'16px',paddingTop:'16px',borderTop:'1px solid var(--line)'}}>
                <a
                  href={`/device/${selected.device_id}`}
                  style={{display:'inline-flex',alignItems:'center',gap:6,fontSize:13,fontWeight:700,color:'var(--primary)'}}
                >
                  View full device report →
                </a>
              </div>
            </div>
          </div>
